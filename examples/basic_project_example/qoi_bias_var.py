...
- create many instances of 4 different QoI estimators. Specifically:
    - random env sample - 1_000 points
    - random env sample - 5_000 points
    - random env sample - 8_000 points
    - random env sample - 10_000 points
- Runs the jobs
...
from problem import PERIOD_LENGTH, brute_force_qoi, dataset, make_exp  # type: ignore[import-not-found]
...
# Add 100 random training points to the the GP
...
qoi_jobs = []
for dataset_size in [1_000, 5_000, 8_000, 10_000]:
    for i in range(20):
...
# and variance for our needs. If even with a large amount of samples there is too much bias/variance, a potential
# solution is to use importance sampling. The tutorial `tutorials/importance_sampling.py` shows how this can be done.
