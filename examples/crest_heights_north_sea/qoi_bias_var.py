"""This explores the bias and variance of QoI estimator with different hyperparameters.

A QoI estimator should be chosen that has sufficiently small bias and variance, as typically one a single instance of
...
for dataset_size in [1_000, 8_000, 16_000]:
    for i in range(40):
        ...
        qoi_jobs.append(
            QoIJob(
                name=f"qoi_dataset_{dataset_size}_{i}",
                qoi=qoi_estimator,
                model=botorch_model_bridge.model.surrogate.model,
                tags={
                    "name": "qoi_gp_high_uncertainty",
...
# %% Plot the different results to understand performance
...
# %% Make a single plot of a specific group
df_subset = df_jobs[df_jobs["dataset_size"] == 1_000]  # noqa: PLR2004
_, axes = plt.subplots(1, 3, figsize=(15, 5))
plot_distribution(df_subset, axes[0], n_hists=5, col_name="samples", brute_force=brute_force_qoi)
...
df_grouped = df_jobs.groupby("dataset_size")
_ = plot_groups(
    df_grouped,
    plotting_funcs=[
        partial(plot_distribution, brute_force=brute_force_qoi),
        partial(plot_best_guess, brute_force=brute_force_qoi),
        partial(plot_col_histogram, col_name="var"),
    ],
)

# %% perform some adhoc analysis
