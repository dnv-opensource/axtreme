...
# TODO(ak 2025-06-12): change import when importance sampling has been moved to src
...
# %% [markdown]
# ### Pick a distribution that you believe captures the noise behavior of your simulator
...
# In `basic_example.py` we use the whole environment dataset for sampler and dataloader. But in this tutorial we want to
# highlight the benefits of using an important sampled dataset instead.
# Importance sampling can be especially helpful and reduce the num_samples required to get an accurate QoI estimation if
# we want to find a tighter/smaller region for our problem automatically. This is the case if the environment data only
# covers a part of the search space.
# TODO(ak 2025-06-11): For the current setup the env data covers more or less evenly the whole search space. To best
# show case the effectiveness of importance sampling the example should be changed.

# %%
# To create an importance sample the underlying distribution for the environment data needs to be known but this is
# often not the case. Therefore, in this example we will approximate it based on the available environment data using a
# kernel density estimation (KDE) approach.


# TODO(ak 2025-06-12): move the importance sampling calculations to a separate script and provide a function to load the
# weights and samples from a file
def approximate_env_data_pdf(x: torch.Tensor, num_training_samples: int = 10_000) -> torch.Tensor:
    """Approximate the probability density function for a given environment dataset using kernel density estimation
    (KDE).

    Args:
        x: environment data
        num_training_samples: number of points fom x used to estimate the pdf

    Returns:
        pdf approximated based on x
    """
    ...
    training_samples = x[torch.randint(0, x.shape[0], (num_training_samples,))]

    _ = grid.fit(training_samples)
    kde_pipeline = grid.best_estimator_
...
# Evaluate PDF
Z = approximate_env_data_pdf(torch.tensor(XY, dtype=torch.float64)).reshape(grid_size, grid_size)

# Plot
fig, ax = plt.subplots(figsize=(6, 5))
ax.scatter(env_data[:, 0], env_data[:, 1], color="grey", s=1)
contours = ax.contour(X, Y, Z, levels=np.logspace(-16, -1, 20), cmap="Purples")
ax.clabel(contours, inline=True, fontsize=8, fmt="%.0e")
ax.set_xlabel("$x_1$")
ax.set_ylabel("$x_2$")

# %%
# Now that we know the distribution of the environment data we can create the importance sample.
...
importance_samples, importance_weights = importance_sampling_distribution_uniform_region(
    approximate_env_data_pdf, region, threshold, num_importance_samples
)


_ = plt.scatter(importance_samples[:, 0], importance_samples[:, 1], color="grey", s=1)
# %%
# Save importance samples and weights
np.save("./examples/demo_2d/data/importance_samples.npy", importance_samples)
np.save("./examples/demo_2d/data/importance_weights.npy", importance_weights)

# %%
# Load existing importance samples and weights
importance_samples = np.load("./examples/demo_2d/data/importance_samples.npy")
importance_weights = np.load("./examples/demo_2d/data/importance_weights.npy")
...
importance_sample_sampler = FixedRandomSampler(importance_dataset, num_samples=n_env_samples, replacement=True, seed=10)
importance_sample_dataloader = DataLoader(importance_dataset, sampler=importance_sample_sampler, batch_size=256)
...
qoi_estimator = MarginalCDFExtrapolation(
    ...
    env_iterable=importance_sample_dataloader,
