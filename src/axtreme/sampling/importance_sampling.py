...
def importance_sampling_from_distribution(
    ...
    below and then generate samples from that distribution and calculate the
...
def importance_sampling_distribution_uniform_region(
    ...
    Args:
        ...
        threshold: The threshold for the real environment distribution.
        num_samples_total: The number of samples to generate.

    Returns:
        A tuple (Tensor, Tensor) where the first is the samples generated from the importance sampling
        distribution, and the second is the importance sampling weights for the samples.
    ...
    Details:
        ...
        This importance sampling approach is useful when:
            - We want to find a tighter/smaller region for our problem automatically. This approach can also
              create automatically non-linear boundaries.
            - You may have disjoint regions.
            - Need the impact of rare events to be explored.

        The approach is sensitive to the following parameters. Often trial and and error (and visualization) is required
        ...
        - Plot and check with a domain expert that the important regions of space are in F. Us a larger F if unsure.
